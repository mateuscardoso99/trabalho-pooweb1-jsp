...
public class UsuarioService {
    ...
    public boolean atualizar(HttpServletRequest req, Usuario usuarioUpdate){
        ...
        Boolean verificarSenha = (!usuarioUpdate.getSenha().equals("")) ? true : false;
        ...
        if (errors.isEmpty()) {
            ...
            usuarioUpdate.setDocumentos(usuarioLogado.getDocumentos());
    ...
    public boolean apagarConta(HttpServletRequest req){
        ...
        try{
            ...
            usuarioLogado.getDocumentos().stream().forEach(doc -> {
                File f = new File(relativePath + File.separator + PATH_DOCS + File.separator + doc.getArquivo());
                f.delete();
            });
